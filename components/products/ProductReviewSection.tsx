...
import { Review, addReview, getProductReviews, hasUserPurchasedProduct } from '@/services/reviewsService';
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        ...
        if (user) {
          const purchased = await hasUserPurchasedProduct(user.uid, productId, user.email);
          // Also check if already reviewed
          const alreadyReviewed = data.some(r => r.userId === user.uid);
          setCanReview(purchased && !alreadyReviewed);
        }
      } catch (error) {
  ...
  return (
    <div className="space-y-8 border-t pt-12">
      ...
      {!user ? (
        ...
        <div className="bg-muted/10 p-6 rounded-2xl border border-dashed border-border/60 text-center">
          <p className="text-sm text-muted-foreground">Only verified buyers can leave a review. Purchase this product to share your experience!</p>
        </div>
      )}
