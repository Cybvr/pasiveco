    ...
    if (!user?.uid) {
      const query =
        selectedOption.planId === 'free'
          ? '?emailPlan=free'
          : `?emailPlan=${selectedOption.planId}&emailPeriod=${selectedOption.billingPeriod}`
      window.location.href = `/auth/register${query}`
      return
    }
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4 px-4">
        ...
        <div className="space-y-2">
          <Badge variant="secondary" className="px-3 py-1 font-bold uppercase tracking-widest text-[10px] bg-primary/10 text-primary border-none">
            {activeEmailPlan?.name || 'Email Plan'}
          </Badge>
          <h1 className="text-2xl font-bold tracking-tight">Email Marketing</h1>
          <p className="text-muted-foreground max-w-sm">
            Welcome to your email command center. Your AI campaign tools are ready.
          </p>
          <p className="text-xs uppercase tracking-widest text-muted-foreground/70">
            {activeEmailPlan?.recipientsLabel}
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-card rounded-3xl ">
        {[
          ...
          <div key={i} className="space-y-3 p-3">
            <div className="h-40 w-full bg-muted/20 rounded-2xl border border-border/50 overflow-hidden">
              <img
                src={feature.image}
                alt={feature.alt}
                className="h-full w-full object-cover"
      ...
      <section ref={pricingRef} className="space-y-5 bg-card rounded-3xl p-6 ">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          <div className="space-y-4">
            ...
            <Button
              className="bg-foreground w-full font-bold uppercase tracking-widest text-xs"
          ...
          {[
            {
              ...
              a: "Email marketing is a powerful marketing channel, a form of direct marketing as well as digital marketing, that uses email to promote your business's products or services."
            },
            {
              q: "What is the Pasive Emailemail marketing service?",
              a: "It's an AI-driven tool integrated into your dashboard that helps you craft, schedule, and automate communications with your customer base."
            },
            {
              q: "How is Pasive Each different from other email marketing apps?",
              a: "Unlike third-party tools, Email is built directly into your store. It understands your products, your customers, and your brand voice natively."
            },
            {
              q: "What are the steps I need to take to send an email campaign?",
              a: "Simply pick a goal, let the AI generate the content and subject line, select your audience segment, and hit send."
            }
          ].map((faq, i) => (
